...
uint8_t* encrypt (const uint8_t* str_to_encrypt){

    uint8_t* u_encrypt = (uint8_t*) malloc (64*sizeof(uint8_t));
    
    memcpy(u_encrypt, str_to_encrypt, 64);
    ...
    struct AES_ctx ctx_e;
    
    AES_init_ctx_iv(&ctx_e, key, iv);

    AES_CBC_encrypt_buffer(&ctx_e, u_encrypt, 64);
